...
fig.suptitle("Hull Suite by InSilico + EMA 200 (1m / 5m / 15m)", fontsize=15)
...
def update(frame):
    for ax, (tf_name, tf_interval) in zip(axes, TIMEFRAMES.items()):
        ...
        ax.set_title(f"{tf_name} | {trend}", color=title_color, fontsize=11)
        ax.grid(alpha=0.25)
